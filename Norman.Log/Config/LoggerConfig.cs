...
namespace Norman.Log.Config
{
	...
	public class LoggerConfig
	{
		...
		/// 默认的LoggerConfig
		/// </summary>
		public static LoggerConfig Default => new LoggerConfig
		{
			LogToFile = new LogToFileConfig
			{
				OnOff = true,
				RootPath = "Logs",
			...
			LogToDatabase = new LogToDatabaseConfig
			{
				OnOff = false,
			...
			LogToServer = new LogToLogServerConfig
			{
				OnOff = false,
		...
		/// </summary>
		/// <returns></returns>
		public static LoggerConfig CreateMockup()
		{
			var random = new Random();
			//TODO:随机生成一个LoggerConfig,当前只是使用了静态值,没有使用随机值
			return new LoggerConfig
			{
				LogToFile = new LogToFileConfig
				{
					...
					CreateFolderRule = LogToFileConfig.CreateFolderRuleEnum.YearFolderMonthFolderDayFolder,
					CreateFileNameRule = LogToFileConfig.CreateFileNameRuleEnum.LoggerNameAndTime,
					MaxSizeKbPerFile = 1024,
					RemainKbTriggerNotify = 1024 * 1024,
					RemainKbTriggerWarn = 1024 * 1024 * 2,
					RemainKbTriggerDanger = 1024 * 1024 * 4,
					RemainKbDelete = 1024 * 1024 * 8,
					MaxLogCountInCache = 1000,
					WriteToFileInterval = TimeSpan.FromMinutes(5),
				},
				LogToDatabase = new LogToDatabaseConfig
				{
					OnOff = false,
		...
		{
			return JsonConvert.DeserializeObject<LoggerConfig>(json);
		}
		
		/// <summary>
		/// 使用json字符串填充当前LoggerConfig
		/// </summary>
		/// <param name="json"></param>
		public void PopulateFromJson(string json)
		{
			...
			LogToServer = config.LogToServer;
		}
		...
		public void LoadFromFile(string path, bool tryCreateIfNotExist = true)
		{
			if (!File.Exists(path))
			{
				if (tryCreateIfNotExist)
				{
					var defaultConfigJson = JsonConvert.SerializeObject(Default, Formatting.Indented);
